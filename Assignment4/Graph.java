...
public class Graph {
    ...
    public void contractEdge(Edge edge_to_contract) {
        ...
        // TODO: implement me
        //merge v into u 
        //remove e=(u,v), remove u
        //update the number of nodes
        //remove edges that had one end equal to u or one end equal to v 
        Iterator<Edge> iterEdges = edges.iterator();  
        List<Edge> toRemove = new ArrayList<Edge>(); 
        while(iterEdges.hasNext()) {
        		Edge oneE = iterEdges.next();      
        		if(oneE != null) {
        			if(((oneE.node_1 == u) && (oneE.node_2 == v))||((oneE.node_2 == u) && (oneE.node_1 == v))) {
            			//has u or v as one end -> remove it 
            			//add to toRemove
            			toRemove.add(oneE); 
            		}
        		}
        }
       edges.removeAll(toRemove); //remove all the u-v edges
       //remove the node 
       this.removeNode(u); 
        
        //replace all occurrences of u with v 
        Iterator<Edge> iterEdges_AfterRemoval = edges.iterator();
        while(iterEdges_AfterRemoval.hasNext()) {
        		Edge oneE = iterEdges_AfterRemoval.next();
        		if(oneE.node_1 == u) {
        			oneE.node_1 = v; //replace it with v 
        		}
        		if(oneE.node_2 == u) {
        			oneE.node_2 = v; 
        		}
